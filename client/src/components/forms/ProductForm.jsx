...
import { InputForm, InputImage, TextAreaForm } from "..";
...
//   const [images, setImages] = useState([]);
  ...
  const {
    ...
    defaultValues: {
      ...
      price: "",
      description: "",
      optionList: [{ name: "", subOptions: [{ id: uuidv4(), name: "" }] }],
    },
  });
    ...
    <div className="h-full flex flex-col">
      ...
      <form className="flex flex-col flex-1 overflow-hidden">
        <div className="flex-1 py-4 overflow-y-scroll scroll-smooth flex flex-col gap-4">
          <div>
            ...
            <Select
              ...
              labelProps={{
                className: "text-orange-main peer-placeholder-shown:text-white",
              }}
              label="Danh mục"
              menuProps={{
          ...
          <InputForm label={"Ảnh sản phẩm"} register={register} id="imageUrl" />
          <InputForm
            label={"Tên sản phẩm"}
              ...
              {fields.map((field, indexOption) => (
                <div key={field.id} className="relative flex gap-2">
                  <div className="relative">
                    <InputForm
                      label={`Lựa chọn ${indexOption + 1}`}
                      register={register}
                      id={`optionList.${indexOption}.name`}
                      validate={{
                        required: "Không được bỏ trống",
                      }}
                      errors={errors?.optionList?.[indexOption]?.name?.message}
                    />
                    <div
                      onClick={() => {
                        if (getValues("optionList").length > 1)
                          remove(indexOption);
                      }}
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    {field?.subOptions?.map((subOption, indexSubOption) => (
                      <div
                      ...
                      >
                        <div className="flex">
                          <InputForm
                            label={`Lựa chọn con ${indexSubOption + 1}`}
                            register={register}
                            ...
                            errors={
                              errors?.optionList?.[indexOption]?.subOptions?.[
                                indexSubOption
                              ]?.name?.message
                            }
                          />
                        ...
                        <div
                          onClick={() => {
                            update(indexOption, {
                              name: getValues(`optionList.${indexOption}.name`),
                              subOptions: [
                                ...getValues(
                                  `optionList.${indexOption}.subOptions`
                                ).filter(
                                  (el, index) => index !== indexSubOption
                                ),
                              ],
                            });
                          }}
        ...
        <div className="w-full flex justify-end py-4 border-t border-orange-main">
          <Button size="lg" color="orange" onClick={handleSubmit(onSubmit)}>
            Xác nhận
          </Button>
        </div>
      </form>
